...
void StateZero()
{
	...
	#elif defined(USE_ONLINE)
	#define MEMPACK_SIZE 0x700000 // 7mb
	#elif defined(USE_RAMEX) && !defined(USE_ONLINE)
	...
	#ifdef USE_ONLINE
	CdlFILE gasmodscdlFile;
	printf("what are you looking for\n");
	DECOMP_CDSYS_SetMode_StreamData();
	if (CdSearchFile(&gasmodscdlFile, "\\GASMODSC.BIN;1") != 0)
	{
		printf("you wont find anything here\n");
		int thingySize;
		DECOMP_LOAD_ReadFile_NoCallback("\\GASMODSC.BIN;1", (void*)0x80700000, &thingySize);
		printf("XD\n");
